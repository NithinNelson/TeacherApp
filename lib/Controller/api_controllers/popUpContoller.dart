      ...
      if (resp['status']['code'] == 200) {
        ...
        for (var sub in recentData.value) {
          if (sub.isprogress ?? true) {
            ...
            if (!isPopupShown && (userRole == UserRole.bothTeacherAndLeader || userRole == UserRole.leader)) {
              if (studentinprogressdata.first.status?.length == 1) {
                TeacherAppPopUps.Trackingpop(
                  title: "Student Coming!",
                  message: "${studentinprogressdata.first.studentName}, from Grade ${studentinprogressdata.first.classs} ${studentinprogressdata.first.batch} on way, Sent By ${studentinprogressdata.first.status?.first.sentBy}",
                  actionName: "Track",
                ...
                TeacherAppPopUps.Trackingpoplate(
                  title: "Late Alert",
                  message: "${studentinprogressdata.first.studentName} has not reached yet.",
                  actionName: "Track",
                  iconColor: Colors.green,
                  timeText: '',
                  sendername: '${studentinprogressdata.first.studentName}',
